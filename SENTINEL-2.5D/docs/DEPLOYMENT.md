# SENTINEL-2.5D Deployment Guide
...
Edit `SENTINEL-2.5D/backend/cpp_cuda/src/sentinel_bridge.cpp` (lines 37-44):
...
**Windows:**
```bash
cd SENTINEL-2.5D/backend/cpp_cuda/build/Release
sentinel_bridge.exe
```
...
```
============================================================
  SENTINEL-2.5D — Defense Tactical Perception Engine (C++/CUDA)
============================================================
...
```bash
cd SENTINEL-2.5D/backend/python_legacy
...
3. Ensure firewall allows port 8000:
   ...
   # Windows
   netsh advfirewall firewall add rule name="SENTINEL-2.5D" dir=in action=allow protocol=TCP localport=8000
   ```
...
Build and run:
```bash
docker build -t sentinel-2.5d .
docker run --gpus all -p 8000:8000 sentinel-2.5d
```
